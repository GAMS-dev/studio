...
void OAIJobsApi::createJob(const QString &model, const QString &_namespace, const QString &stdout_filename, const QList<QString> &text_entries, const QList<QString> &stream_entries, const QList<QString> &arguments, const OAIHttpFileElement &model_data, const OAIHttpFileElement &data, const OAIHttpFileElement &inex_file) {
    ...
    if (fullPath.indexOf("?") > 0)
        ...
        fullPath.append("?");
    fullPath.append(QUrl::toPercentEncoding("namespace")).append("=").append(QUrl::toPercentEncoding(::OpenAPI::toStringValue(_namespace)));
...
void OAIJobsApi::getJob(const QString &token, const QString &x_fields) {
    ...
    if (x_fields != nullptr) {
        input.headers.insert("X-Fields", x_fields);
    }
...
void OAIJobsApi::getJobZipCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        emit getJobZipSignal();
        emit getJobZipSignalFull(worker);
    } else {
        emit getJobZipSignalE(error_type, error_str);
