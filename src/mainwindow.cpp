      ...
      mMiroDeployDialog(new miro::MiroDeployDialog(this))
{
    ...
    connect(mLicenseFetcher.get(), &support::LicenseFetcher::error, this, &MainWindow::appendSystemLogError);
    connect(mLicenseFetcher.get(), &support::LicenseFetcher::info, this, &MainWindow::appendSystemLogInfo);
                                ...
                                if (optionName.isEmpty()) {
                                    ...
                                    if (optionEdit->fileKind()==FileKind::Pf)
                                     ...
                                     if (optionName.isEmpty())
                                         ...
                                         mHelpWidget->on_helpContentRequested( help::DocumentType::GamsCall, optionName);
                            } else if (connect::ConnectEditor* editor = ViewHelper::toGamsConnectEditor(mRecent.editor())) {
                                       mHelpWidget->on_helpContentRequested( help::DocumentType::Connect,
                                                                             editor->getSelectedAgentName(),
                                                                             editor->getSelectedAttributeName());
    ...
    if (ViewHelper::toCodeEdit(edit)) {
        ...
        connect(fileMeta, &FileMeta::openFile, this, [this](const QString &filePath, NodeId projectId) {
            if (QFile::exists(filePath)) {
                PExProjectNode *project = mRecent.project();
                if (project) {
                    emit openFilePath(filePath, project, ogFindGroup, true);
                }
            }
        });
    }
...
{
    FontGroup fg;
    if (mResultsView->isAncestorOf(widget)) {
