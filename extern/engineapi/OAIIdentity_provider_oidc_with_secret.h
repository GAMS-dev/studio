/**
 * GAMS Engine
 * With GAMS Engine you can register and solve GAMS models. It has a namespace management system, so you can restrict your users to certain models.
 *
 * The version of the OpenAPI document: 25.04.23
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIIdentity_provider_oidc_with_secret.h
 *
 * 
 */

#ifndef OAIIdentity_provider_oidc_with_secret_H
#define OAIIdentity_provider_oidc_with_secret_H

#include <QJsonObject>

#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIIdentity_provider_oidc_with_secret : public OAIObject {
public:
    OAIIdentity_provider_oidc_with_secret();
    OAIIdentity_provider_oidc_with_secret(QString json);
    ~OAIIdentity_provider_oidc_with_secret() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getAuthorizationEndpoint() const;
    void setAuthorizationEndpoint(const QString &authorization_endpoint);
    bool is_authorization_endpoint_Set() const;
    bool is_authorization_endpoint_Valid() const;

    QString getDeviceAuthorizationEndpoint() const;
    void setDeviceAuthorizationEndpoint(const QString &device_authorization_endpoint);
    bool is_device_authorization_endpoint_Set() const;
    bool is_device_authorization_endpoint_Valid() const;

    QString getDeviceClientId() const;
    void setDeviceClientId(const QString &device_client_id);
    bool is_device_client_id_Set() const;
    bool is_device_client_id_Valid() const;

    QString getDeviceClientSecret() const;
    void setDeviceClientSecret(const QString &device_client_secret);
    bool is_device_client_secret_Set() const;
    bool is_device_client_secret_Valid() const;

    QString getEndSessionEndpoint() const;
    void setEndSessionEndpoint(const QString &end_session_endpoint);
    bool is_end_session_endpoint_Set() const;
    bool is_end_session_endpoint_Valid() const;

    QList<QString> getExtraClientIds() const;
    void setExtraClientIds(const QList<QString> &extra_client_ids);
    bool is_extra_client_ids_Set() const;
    bool is_extra_client_ids_Valid() const;

    bool isHasDeviceClientSecret() const;
    void setHasDeviceClientSecret(const bool &has_device_client_secret);
    bool is_has_device_client_secret_Set() const;
    bool is_has_device_client_secret_Valid() const;

    bool isHasWebUiClientSecret() const;
    void setHasWebUiClientSecret(const bool &has_web_ui_client_secret);
    bool is_has_web_ui_client_secret_Set() const;
    bool is_has_web_ui_client_secret_Valid() const;

    bool isHidden() const;
    void setHidden(const bool &hidden);
    bool is_hidden_Set() const;
    bool is_hidden_Valid() const;

    QString getIssuer() const;
    void setIssuer(const QString &issuer);
    bool is_issuer_Set() const;
    bool is_issuer_Valid() const;

    QString getJwksUri() const;
    void setJwksUri(const QString &jwks_uri);
    bool is_jwks_uri_Set() const;
    bool is_jwks_uri_Valid() const;

    QString getLabel() const;
    void setLabel(const QString &label);
    bool is_label_Set() const;
    bool is_label_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    QList<QString> getScopes() const;
    void setScopes(const QList<QString> &scopes);
    bool is_scopes_Set() const;
    bool is_scopes_Valid() const;

    QString getTokenEndpoint() const;
    void setTokenEndpoint(const QString &token_endpoint);
    bool is_token_endpoint_Set() const;
    bool is_token_endpoint_Valid() const;

    QList<QString> getTokenEndpointAuthMethodsSupported() const;
    void setTokenEndpointAuthMethodsSupported(const QList<QString> &token_endpoint_auth_methods_supported);
    bool is_token_endpoint_auth_methods_supported_Set() const;
    bool is_token_endpoint_auth_methods_supported_Valid() const;

    QString getWebUiClientId() const;
    void setWebUiClientId(const QString &web_ui_client_id);
    bool is_web_ui_client_id_Set() const;
    bool is_web_ui_client_id_Valid() const;

    QString getWebUiClientSecret() const;
    void setWebUiClientSecret(const QString &web_ui_client_secret);
    bool is_web_ui_client_secret_Set() const;
    bool is_web_ui_client_secret_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_authorization_endpoint;
    bool m_authorization_endpoint_isSet;
    bool m_authorization_endpoint_isValid;

    QString m_device_authorization_endpoint;
    bool m_device_authorization_endpoint_isSet;
    bool m_device_authorization_endpoint_isValid;

    QString m_device_client_id;
    bool m_device_client_id_isSet;
    bool m_device_client_id_isValid;

    QString m_device_client_secret;
    bool m_device_client_secret_isSet;
    bool m_device_client_secret_isValid;

    QString m_end_session_endpoint;
    bool m_end_session_endpoint_isSet;
    bool m_end_session_endpoint_isValid;

    QList<QString> m_extra_client_ids;
    bool m_extra_client_ids_isSet;
    bool m_extra_client_ids_isValid;

    bool m_has_device_client_secret;
    bool m_has_device_client_secret_isSet;
    bool m_has_device_client_secret_isValid;

    bool m_has_web_ui_client_secret;
    bool m_has_web_ui_client_secret_isSet;
    bool m_has_web_ui_client_secret_isValid;

    bool m_hidden;
    bool m_hidden_isSet;
    bool m_hidden_isValid;

    QString m_issuer;
    bool m_issuer_isSet;
    bool m_issuer_isValid;

    QString m_jwks_uri;
    bool m_jwks_uri_isSet;
    bool m_jwks_uri_isValid;

    QString m_label;
    bool m_label_isSet;
    bool m_label_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    QList<QString> m_scopes;
    bool m_scopes_isSet;
    bool m_scopes_isValid;

    QString m_token_endpoint;
    bool m_token_endpoint_isSet;
    bool m_token_endpoint_isValid;

    QList<QString> m_token_endpoint_auth_methods_supported;
    bool m_token_endpoint_auth_methods_supported_isSet;
    bool m_token_endpoint_auth_methods_supported_isValid;

    QString m_web_ui_client_id;
    bool m_web_ui_client_id_isSet;
    bool m_web_ui_client_id_isValid;

    QString m_web_ui_client_secret;
    bool m_web_ui_client_secret_isSet;
    bool m_web_ui_client_secret_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIIdentity_provider_oidc_with_secret)

#endif // OAIIdentity_provider_oidc_with_secret_H
